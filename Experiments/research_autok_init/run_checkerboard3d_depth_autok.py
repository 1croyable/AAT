...
OUT_DIR = Path("./checkerboard3d_depth_autok")
...
DEPTH_LIST = list(range(1, 9))  # 1..8 layers
EXTRA_DIMS = 3
NUM_CLASSES = 2
...
MAX_DEPTH_FOR_COLUMNS = max(DEPTH_LIST)
...
@torch.no_grad()
def layer_diagnostics(model: AATField, x: torch.Tensor, device: torch.device, max_samples: int = 4096) -> Dict[str, object]:
    model.eval()
    ...
    for li in range(1, MAX_DEPTH_FOR_COLUMNS + 1):
        for name in [
            "children",
            "sigma_mean",
            "sigma_min",
...
def write_summary(results_csv: Path, summary_csv: Path) -> None:
    ...
    for i in range(0, MAX_DEPTH_FOR_COLUMNS + 1):
        fields.append(f"val_fisher_l{i}")

    for i in range(1, MAX_DEPTH_FOR_COLUMNS + 1):
        fields.extend([
            f"layer{i}_children",
            f"layer{i}_sigma_mean",
            f"layer{i}_gate_active",
            f"layer{i}_move_norm",
            f"layer{i}_alpha_eff",
        ])
...
def run_id(data: DataBundle, spec: RunSpec) -> str:
    return (
        f"{data.name}__Depth{spec.layers}_extra{spec.extra_dims}"
...
def train_one(spec: RunSpec, data: DataBundle, device: torch.device) -> Dict[str, object]:
    ...
    for i in range(0, MAX_DEPTH_FOR_COLUMNS + 1):
        row[f"train_fisher_l{i}"] = ""
        row[f"val_fisher_l{i}"] = ""
        row[f"test_fisher_l{i}"] = ""
...
def build_specs() -> List[RunSpec]:
    ...
    for depth in DEPTH_LIST:
        specs.append(RunSpec(
            model=f"AAT-Depth{depth}-extra{EXTRA_DIMS}-AutoK-max{AUTO_MAX_CHILDREN}",
            seed=SEED,
            layers=int(depth),
...
def main() -> None:
    ...
    print(
        ...
        f"device={device} depths={DEPTH_LIST} autok_max={AUTO_MAX_CHILDREN} "
