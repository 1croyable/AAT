# -*- coding: utf-8 -*-
"""
run_airline_field_coordinate_relu.py

Field-coordinate ReLU AAT experiment on Airline Satisfaction.

Goal
----
Test post-transport hard folding on field-defined coordinates:

    conservative / potential-style anchor field transport
    + post-transport field-coordinate ReLU / reflection
...
Recommended placement:
    C:/Projets/AATField/Experiments/research-activation/run_airline_field_coordinate_relu.py
...
class ConservativeAATField(AATField):
    ...
    VALID_ACTIVATIONS = {
        ...
        # Baseline / reference.
        "post_relu",
        # Old diagnostics kept optional, not default.
        "energy_drop",
        "work_clip",
        "boundary_push",
        "child_axis",
        # New post-transport hard slicing modes.
        "child_score_relu",
        "child_hard_fold",
        "winner_commit",
        "top2_score_slice",
        # Field-coordinate hard folds: project/reflect a scalar field coordinate, not anchor commit.
        "competition_relu",
        "competition_abs",
        "competition_hard",
    }
    ...
    def __init__(
        ...
        *,
        ...
        slice_strength: float = 0.20,
        score_margin: float = 0.0,
        coord_margin: float = 0.50,
        ...
        self.slice_strength = float(slice_strength)
        self.score_margin = float(score_margin)
        self.coord_margin = float(coord_margin)
    ...
    def _field_step(self, layer, z: torch.Tensor, *, apply_cap: bool = True) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        if self.core_mode == "potential":
            ...
            sigma2 = sigma.view(1, -1).square().clamp_min(1e-8)
            weighted_anchor = (alpha / sigma2) @ anchors
            weighted_sum = (alpha / sigma2).sum(dim=1, keepdim=True)
    ...
    def _child_score_relu_activation(self, layer, z_mid: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """
        Post-transport field-response ReLU.

        This is deliberately NOT a pre-transport gate. The conservative field first
        moves the point to z_mid. Only then do we compute child-vs-parent response
        and add a second, non-conservative fold along the selected child axis.

        score gap:
            g = score(best_child, z_mid) - score(its_parent, z_mid)

        activation:
            amount = ReLU(g - margin)
            z_next = z_mid + slice_strength * amount * unit(parent -> child)
        """
        C = int(layer.num_classes)
        K = int(layer.children_per_class)
        if K < 1:
            z0 = z_mid.new_zeros(z_mid.shape[0])
            return z_mid, {
                "child_score_push_norm": z0,
                "child_score_gap": z0,
                "child_score_active_rate": z0,
            }

        scores, _, _, _ = self._potential_scores(layer, z_mid)
        child_scores = scores[:, C:]
        child_score, idx = child_scores.max(dim=1)  # over C*K children
        parent_idx = idx // K
        parent_score = scores[:, :C].gather(1, parent_idx.view(-1, 1)).squeeze(1)
        ...
        amount = F.relu(gap - float(self.score_margin)).view(-1, 1)
        push = float(self.slice_strength) * amount * u
        z_next = z_mid + push
        return z_next, {
            "child_score_push_norm": push.norm(dim=-1),
            "child_score_gap": gap,
            "child_score_active_rate": (gap > float(self.score_margin)).float(),
        }

    def _child_hard_fold_activation(self, layer, z_mid: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """
        Hard child-response fold.

        Same post-transport score decision as child_score_relu, but the branch is
        hard: if the best child already beats its parent, commit by a fixed amount
        along that child axis. This intentionally creates a piecewise field region.
        """
        C = int(layer.num_classes)
        K = int(layer.children_per_class)
        if K < 1:
            z0 = z_mid.new_zeros(z_mid.shape[0])
            return z_mid, {
                "child_hard_push_norm": z0,
                "child_hard_gap": z0,
                "child_hard_active_rate": z0,
            }

        scores, _, _, _ = self._potential_scores(layer, z_mid)
        child_scores = scores[:, C:]
        child_score, idx = child_scores.max(dim=1)
        parent_idx = idx // K
        parent_score = scores[:, :C].gather(1, parent_idx.view(-1, 1)).squeeze(1)
        gap = child_score - parent_score
        ...
        active = (gap > float(self.score_margin)).float().view(-1, 1)
        push = float(self.slice_strength) * active * u
        z_next = z_mid + push
        return z_next, {
            "child_hard_push_norm": push.norm(dim=-1),
            "child_hard_gap": gap,
            "child_hard_active_rate": active.squeeze(1),
        }

    def _winner_commit_activation(self, layer, z_mid: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """
        Hard winner-field commit.

        After conservative transport, select the strongest anchor response at
        z_mid and move a small fraction toward that winner. This creates an
        argmax-defined field partition, similar in spirit to ReLU choosing a
        branch after a linear response.
        """
        scores, _, anchors, _ = self._potential_scores(layer, z_mid)
        _, idx = scores.max(dim=1)
        winner = anchors.index_select(0, idx)
        push = float(self.slice_strength) * (winner - z_mid)
        z_next = z_mid + push
        C = int(layer.num_classes)
        return z_next, {
            "winner_commit_push_norm": push.norm(dim=-1),
            "winner_is_child_rate": (idx >= C).float(),
        }

    def _top2_score_slice_activation(self, layer, z_mid: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """
        Top-2 response slicing.

        After conservative transport, compare the strongest two anchor responses.
        If the winner is not sufficiently separated from its competitor, push the
        point through the local winner-vs-runner-up slicing direction. This is an
        anchor-response version of margin sharpening, not a pre-move gate.
        """
        scores, _, anchors, _ = self._potential_scores(layer, z_mid)
        if anchors.shape[0] < 2:
            z0 = z_mid.new_zeros(z_mid.shape[0])
            return z_mid, {
                "top2_slice_push_norm": z0,
                "top2_score_gap": z0,
                "top2_uncertain_rate": z0,
            }
        vals, idx = torch.topk(scores, k=2, dim=1, largest=True)
        a1 = anchors.index_select(0, idx[:, 0])
        a2 = anchors.index_select(0, idx[:, 1])
        u = a1 - a2
        u = u / u.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        gap = vals[:, 0] - vals[:, 1]
        amount = F.relu(float(self.score_margin) - gap).view(-1, 1)
        push = float(self.slice_strength) * amount * u
        z_next = z_mid + push
        return z_next, {
            "top2_slice_push_norm": push.norm(dim=-1),
            "top2_score_gap": gap,
            "top2_uncertain_rate": (gap < float(self.score_margin)).float(),
        }

    def _competition_coordinate_activation(self, layer, z_mid: torch.Tensor, *, mode: str) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """
        Field-coordinate ReLU / reflection on the top1-vs-top2 score gap.

        This is not a gate and not a winner commit:
            1) conservative transport has already produced z_mid;
            2) compute a scalar field coordinate

                   phi(z) = score_top1(z) - score_top2(z) - coord_margin

            3) if phi < 0, directly project / reflect z_mid across phi=0 using
               the local normal grad(phi).  We do NOT move toward the winner anchor.

        Modes
        -----
        competition_relu:
            negative side is projected to phi=0, like ReLU clamps negative responses.

        competition_abs:
            negative side is reflected across phi=0, like abs folds the negative side.

        competition_hard:
            hard binary fold: if phi<0, use a fixed projection amount equal to coord_margin.
            This is intentionally more brutal, only for quick diagnosis.
        """
        scores, _, anchors, sigma = self._potential_scores(layer, z_mid)
        if anchors.shape[0] < 2:
            z0 = z_mid.new_zeros(z_mid.shape[0])
            return z_mid, {
                "competition_phi": z0,
                "competition_gap": z0,
                "competition_active_rate": z0,
                "competition_fold_norm": z0,
                "competition_grad_norm": z0,
            }

        vals, idx = torch.topk(scores, k=2, dim=1, largest=True)
        i1, i2 = idx[:, 0], idx[:, 1]
        a1 = anchors.index_select(0, i1)
        a2 = anchors.index_select(0, i2)
        s1 = sigma.index_select(0, i1).view(-1, 1).square().clamp_min(1e-8)
        s2 = sigma.index_select(0, i2).view(-1, 1).square().clamp_min(1e-8)

        gap = vals[:, 0] - vals[:, 1]
        phi = gap - float(self.coord_margin)

        # grad_z(score_i) = (a_i - z) / sigma_i^2
        grad_phi = (a1 - z_mid) / s1 - (a2 - z_mid) / s2
        grad_norm2 = grad_phi.square().sum(dim=1, keepdim=True).clamp_min(1e-8)
        negative = (phi < 0).float().view(-1, 1)
        deficit = F.relu(-phi).view(-1, 1)

        if mode == "competition_relu":
            # First-order projection to phi=0.
            fold = deficit * grad_phi / grad_norm2
        elif mode == "competition_abs":
            # Reflection across phi=0.
            fold = 2.0 * deficit * grad_phi / grad_norm2
        elif mode == "competition_hard":
            # A cruder diagnostic: fixed hard fold when inside the uncertain strip.
            fold = negative * float(self.coord_margin) * grad_phi / grad_phi.norm(dim=1, keepdim=True).clamp_min(1e-8)
        else:
            raise RuntimeError(f"bad competition coordinate mode={mode}")

        z_next = z_mid + fold
        return z_next, {
            "competition_phi": phi,
            "competition_gap": gap,
            "competition_active_rate": negative.squeeze(1),
            "competition_fold_norm": fold.norm(dim=1),
            "competition_grad_norm": grad_phi.norm(dim=1),
        }
    ...
    def _apply_activation(self, layer, z: torch.Tensor, z_mid: torch.Tensor, info: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        if mode == "post_relu":
            z_next = F.relu(z_mid)
            return z_next, {"activation_zero_rate": (z_next.abs() <= 1e-8).float().mean(dim=1)}
        if mode == "energy_drop":
            # Strict potential rule: keep a post-transport state only if it lowers
            # the anchor potential. This is intentionally hard, like a field-ReLU.
            e0 = info["energy"]
            e1 = self._potential_energy(layer, z_mid)
            accept = (e1 < e0).float().view(-1, 1)
            z_next = accept * z_mid + (1.0 - accept) * z
            return z_next, {"energy_delta": e1 - e0, "activation_accept_rate": accept.squeeze(1)}
        if mode == "work_clip":
            # Keep a step only if the local field after the step still agrees with
            # the executed move. Otherwise reject the step after observing it.
            z_probe, info_probe = self._field_step(layer, z_mid, apply_cap=True)
            f_mid = z_probe - z_mid
            work = (info["move"] * f_mid).sum(dim=-1)
            accept = (work > 0).float().view(-1, 1)
            z_next = accept * z_mid + (1.0 - accept) * z
            return z_next, {"field_work": work, "activation_accept_rate": accept.squeeze(1)}
        if mode == "boundary_push":
            return self._boundary_push_activation(layer, z_mid)
        if mode == "child_axis":
            return self._child_axis_activation(layer, z_mid, info["alpha"])
        if mode == "child_score_relu":
            return self._child_score_relu_activation(layer, z_mid)
        if mode == "child_hard_fold":
            return self._child_hard_fold_activation(layer, z_mid)
        if mode == "winner_commit":
            return self._winner_commit_activation(layer, z_mid)
        if mode == "top2_score_slice":
            return self._top2_score_slice_activation(layer, z_mid)
        if mode in {"competition_relu", "competition_abs", "competition_hard"}:
            return self._competition_coordinate_activation(layer, z_mid, mode=mode)
...
def collect_diagnostics(model: ConservativeAATField, x: torch.Tensor, device: torch.device, batch_size: int = 2048) -> Dict[str, Any]:
    ...
    for _ in model.layers:
        per_layer.append({
            ...
            "move_state_ratio": [],
            "energy_delta": [],
            "activation_accept_rate": [],
            "field_work": [],
            "boundary_push_norm": [],
            "boundary_margin_value": [],
            "child_axis_push_norm": [],
            "child_axis_positive_rate": [],
            "activation_zero_rate": [],
            "child_score_push_norm": [],
            "child_score_gap": [],
            "child_score_active_rate": [],
            "child_hard_push_norm": [],
            "child_hard_gap": [],
            "child_hard_active_rate": [],
            "winner_commit_push_norm": [],
            "winner_is_child_rate": [],
            "top2_slice_push_norm": [],
            "top2_score_gap": [],
            "top2_uncertain_rate": [],
            "competition_phi": [],
            "competition_gap": [],
            "competition_active_rate": [],
            "competition_fold_norm": [],
            "competition_grad_norm": [],
        })
    ...
    for key in [
        "state_norm", "move_norm", "move_state_ratio",
        "energy_delta", "activation_accept_rate", "field_work",
        "boundary_push_norm", "child_axis_push_norm", "activation_zero_rate",
        "child_score_push_norm", "child_score_gap", "child_score_active_rate",
        "child_hard_push_norm", "child_hard_gap", "child_hard_active_rate",
        "winner_commit_push_norm", "winner_is_child_rate",
        "top2_slice_push_norm", "top2_score_gap", "top2_uncertain_rate",
        "competition_phi", "competition_gap", "competition_active_rate",
        "competition_fold_norm", "competition_grad_norm",
    ]:
...
def train_one(
    *,
    ...
    model = ConservativeAATField(
        ...
        slice_strength=float(args.slice_strength),
        score_margin=float(args.score_margin),
        coord_margin=float(args.coord_margin),
...
def append_csv(path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    first = [
        ...
        "boundary_margin", "boundary_strength", "child_axis_strength", "slice_strength", "score_margin", "coord_margin", "error",
    ]
    keys = []
    for r in rows:
        for k in r.keys():
...
def write_summary(path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        fields = [
            ...
            "diag_mean_move_norm", "diag_mean_move_state_ratio", "diag_mean_energy_delta",
            "diag_mean_activation_accept_rate", "diag_mean_boundary_push_norm",
            "diag_mean_child_axis_push_norm", "diag_mean_activation_zero_rate",
            "diag_mean_competition_fold_norm", "diag_mean_competition_gap",
            "diag_mean_competition_phi", "diag_mean_competition_active_rate",
        ]
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Field-coordinate ReLU AAT Airline experiment")
    p.add_argument("--data-dir", type=str, default="../data/AirlineSatisfaction")
    p.add_argument("--out-dir", type=str, default="./airline_field_coordinate_relu")
    ...
    p.add_argument(
        "--activation-modes",
        type=str,
        default="competition_relu,competition_abs,competition_hard",
        help="competition_relu,competition_abs,competition_hard plus optional old modes: none,post_relu,child_axis,boundary_push",
    )
    p.add_argument("--layers", type=str, default="2,4")
    ...
    p.add_argument("--slice-strength", type=float, default=0.20, help="Post-transport slicing/folding strength for hard field activations.")
    p.add_argument("--score-margin", type=float, default=0.0, help="Score threshold/margin used by old child/top2 slicing modes.")
    p.add_argument("--coord-margin", type=float, default=0.50, help="Margin for field-coordinate ReLU: phi = top1_score - top2_score - coord_margin.")
