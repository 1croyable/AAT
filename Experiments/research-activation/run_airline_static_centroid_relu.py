# -*- coding: utf-8 -*-
"""
run_airline_static_centroid_relu.py

Static-centroid reset AAT experiment on Airline Satisfaction.

Goal
----
Test a simpler post-transport activation:

    judge z_mid per dimension
    gate by a layer-level static centroid r
    reset failed dimensions to r

Unlike the previous dynamic field-center version, r is NOT sample-dependent.
...
Recommended placement:
    C:/Projets/AATField/Experiments/research-activation/run_airline_static_centroid_relu.py
...
class ConservativeAATField(AATField):
    ...
    VALID_ACTIVATIONS = {
        # z_mid is always computed first by the potential field.
        # Then a simple layer-level centroid r is used as the reset value / zero point.
        # r is not sample-dependent. This is intentionally close to ReLU:
        #     z_next = r + ReLU(z_mid - r) = max(z_mid, r)
        "parent_centroid_relu",    # r = mean(parent anchors)
        "anchor_centroid_relu",    # r = mean(all parent + child anchors)
        "coverage_centroid_relu",  # r = sigma-weighted mean(all anchors)
    }
    ...
    def __init__(
        ...
        *,
        ...
        activation_mode: str = "anchor_centroid_relu",
    ...
    def _field_step(self, layer, z: torch.Tensor, *, apply_cap: bool = True) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        if self.core_mode == "potential":
            ...
            sigma2 = sigma.view(1, -1).square().clamp_min(1e-8)
            # Transport vector is the potential gradient.
            ...
            # Activation reference: the simple soft field barycenter, not a winner.
            # This is the field-related reset point r.
            field_center = alpha @ anchors
    ...
    def _static_centroid_reference(self, layer, z_mid: torch.Tensor) -> torch.Tensor:
        """Return one layer-level reset point r, expanded to batch shape.

        This is intentionally simpler than the dynamic field center:

        parent_centroid_relu:
            r = mean(parent anchors)

        anchor_centroid_relu:
            r = mean(parent + child anchors)

        coverage_centroid_relu:
            r = sigma-weighted mean(parent + child anchors)

        The center can be detached from the activation path by default.  This
        keeps the reset reference closer to ReLU's fixed zero while still making
        the value field-related because it is recomputed from layer anchors.
        """
        anchors, sigma, _ = self._anchors_sigma(layer)
        mode = self.activation_mode

        if mode == "parent_centroid_relu":
            r = layer.parents.mean(dim=0, keepdim=True)
        elif mode == "anchor_centroid_relu":
            r = anchors.mean(dim=0, keepdim=True)
        elif mode == "coverage_centroid_relu":
            w = sigma.view(-1, 1).clamp_min(1e-8)
            r = (w * anchors).sum(dim=0, keepdim=True) / w.sum().clamp_min(1e-8)
        else:
            raise RuntimeError(f"bad activation_mode={mode}")

        if self.detach_center:
            r = r.detach()
        return r.expand_as(z_mid)
    ...
    def _apply_activation(self, layer, z: torch.Tensor, z_mid: torch.Tensor, info: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        # Judge z_mid after transport.  Failed dimensions reset to one
        # static layer-level centroid r.  This is the simplest field-related
        # ReLU form: z_next = r + ReLU(z_mid - r).
        r = self._static_centroid_reference(layer, z_mid)
        delta = z_mid - r
        ...
        reset = z_next - z_mid
        return z_next, {
            "field_center_norm": r.norm(dim=-1),
            "field_center_delta_norm": delta.norm(dim=-1),
            "field_gate_active_rate": active.float().mean(dim=1),
            "field_reset_norm": reset.norm(dim=-1),
        }
...
def collect_diagnostics(model: ConservativeAATField, x: torch.Tensor, device: torch.device, batch_size: int = 2048) -> Dict[str, Any]:
    ...
    for _ in model.layers:
        per_layer.append({
            ...
            "move_state_ratio": [],
            "field_center_norm": [],
            "field_center_delta_norm": [],
            "field_gate_active_rate": [],
            "field_reset_norm": [],
        })
    ...
    for key in ["state_norm", "move_norm", "move_state_ratio", "field_center_norm", "field_center_delta_norm", "field_gate_active_rate", "field_reset_norm"]:
...
def train_one(
    *,
    ...
    model = ConservativeAATField(
        ...
        detach_center=not bool(args.no_detach_center),
    ).to(device)
    ...
    row: Dict[str, Any] = {
        ...
        "boundary_strength": float(args.boundary_strength),
        "child_axis_strength": float(args.child_axis_strength),
        "center_detached": not bool(args.no_detach_center),
        "params": None,
        "total_children": None,
...
def write_summary(path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        fields = [
            ...
            "diag_mean_field_center_norm", "diag_mean_field_center_delta_norm",
            "diag_mean_field_gate_active_rate", "diag_mean_field_reset_norm",
        ]
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Static-centroid ReLU AAT Airline experiment")
    p.add_argument("--data-dir", type=str, default="../data/AirlineSatisfaction")
    p.add_argument("--out-dir", type=str, default="./airline_static_centroid_relu")
    ...
    p.add_argument(
        "--activation-modes",
        type=str,
        default="anchor_centroid_relu,coverage_centroid_relu,parent_centroid_relu",
        help="anchor_centroid_relu,coverage_centroid_relu,parent_centroid_relu",
    )
    ...
    p.add_argument("--no-detach-center", action="store_true", help="Allow activation reset center to receive direct gradients")
