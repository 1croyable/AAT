# -*- coding: utf-8 -*-
"""
run_airline_field_gate_reset.py

Field-gate reset AAT experiment on Airline Satisfaction.

Goal
----
Test an extremely simple post-transport activation:

    judge z_mid per dimension
    gate by a field-derived reference r
...
Recommended placement:
    C:/Projets/AATField/Experiments/research-activation/run_airline_field_gate_reset.py
...
class ConservativeAATField(AATField):
    ...
    VALID_ACTIVATIONS = {
        ...
        # Then a field reference r(z_mid) = softmax_field_barycenter(z_mid)
        # is used as the reset value / zero point.
        "field_center_relu",   # z_next = r + ReLU(z_mid - r) = max(z_mid, r)
        "field_center_abs",    # z_next = r + abs(z_mid - r)
        "move_sign_reset",     # keep z_mid dim if move_d >= 0 else reset to r_d
        "center_hard_reset",   # keep z_mid dim if z_mid_d >= r_d else reset to r_d
    }
    ...
    def __init__(
        ...
        *,
        ...
        activation_mode: str = "field_center_relu",
    ...
    def _field_reference_at_mid(self, layer, z_mid: torch.Tensor) -> torch.Tensor:
        """Return the field-related reset point r for z_mid.

        This is intentionally simple:
            r(z_mid) = sum_i softmax(score_i(z_mid)) * anchor_i

        It is not top1/top2, not a winner commit, not a margin rule.  It is just
        the barycenter of the same anchor field observed at the post-transport
        position.  Activation can then use r as the per-dimension zero/reset
        value, analogous to ReLU's zero.
        """
        scores, _, anchors, _ = self._potential_scores(layer, z_mid)
        alpha_mid = torch.softmax(scores, dim=-1)
        return alpha_mid @ anchors
    ...
    def _apply_activation(self, layer, z: torch.Tensor, z_mid: torch.Tensor, info: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        # All modes judge z_mid after transport.  The failed value is reset to
        # r, where r is a field-derived reference point.
        r = self._field_reference_at_mid(layer, z_mid)
        delta = z_mid - r

        if mode == "field_center_relu":
            # Per-dimension ReLU around the field center:
            #   z_next_d = r_d + ReLU(z_mid_d - r_d)
            #            = max(z_mid_d, r_d)
            z_next = r + F.relu(delta)
            active = (delta > 0).float()
        elif mode == "field_center_abs":
            # Hard reflection around the field center.
            z_next = r + delta.abs()
            active = torch.ones_like(delta)
        elif mode == "move_sign_reset":
            # Gate by the field displacement itself, not by anchor score:
            #   if move_d >= 0 keep the transported coordinate;
            #   otherwise reset that coordinate to the field center r_d.
            gate = (info["move"] >= 0).to(dtype=z_mid.dtype)
            z_next = gate * z_mid + (1.0 - gate) * r
            active = gate
        elif mode == "center_hard_reset":
            # Same output as field_center_relu, but written as an explicit hard
            # gate.  This is useful to compare with the ReLU expression.
            gate = (delta >= 0).to(dtype=z_mid.dtype)
            z_next = gate * z_mid + (1.0 - gate) * r
            active = gate
        else:
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Field-gate reset AAT Airline experiment")
    p.add_argument("--data-dir", type=str, default="../data/AirlineSatisfaction")
    p.add_argument("--out-dir", type=str, default="./airline_field_gate_reset")
    ...
    p.add_argument(
        "--activation-modes",
        type=str,
        default="field_center_relu,move_sign_reset,center_hard_reset",
        help="field_center_relu,field_center_abs,move_sign_reset,center_hard_reset",
    )
