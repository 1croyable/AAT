...
V4 changes:
  - use the same Auto-K structure as initialize.py
  - use RMS distance for high-dimensional Gaussian kernels
...
class GaussianFieldLayer(nn.Module):
    ...
    def field(self, z: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        dim_scale = float(max(self.state_dim, 1))
        coef = self.charge.view(1, -1) * k / (sigma2 * dim_scale)
...
def main() -> None:
    ...
    print("=" * 110)
    print("Progressive Parentless Additive Gaussian AAT | flattened MNIST | dimension-normalized v4")
    print("=" * 110)
    print(
    ...
    print(
        "field: Phi=sum c_j exp(-RMSDist(z,a_j)^2/(2 sigma_j^2)); "
    ...
    print(
        ...
        f"sigma_init_mode={args.sigma_init_mode}"
    )
