...
class GaussianAATStack(nn.Module):
    def __init__(self, layers: Sequence[GaussianFieldLayer], *, activation_angle_degrees: float = 45.0):
        super().__init__()
    ...
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor], List[Dict[str, torch.Tensor]]]:
        ...
        for layer in self.layers:
            ...
            z_act = self._field_rotated_local_relu(z_mid, center, grad)
            info = dict(info)
            info["activation_move_norm"] = (z_act - z_mid).norm(dim=1)
            moves.append(move)
            infos.append(info)
            z = z_act
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--activation-angle-degrees", type=float, default=45.0)
...
def main() -> None:
    ...
    print("=" * 110)
    print("Progressive Parentless Additive Gaussian AAT | flattened MNIST | activation-aligned v5")
    print("=" * 110)
    print(
    ...
    print(
        "training: progressive init; each layer output includes local activation; "
        "previous layers continue with smaller LR; no dropout, no scheduler, no softmax"
    )
    print(
        ...
        f"sigma_init_mode={args.sigma_init_mode}, activation_angle={args.activation_angle_degrees}"
    )
    ...
    stack = GaussianAATStack([layer1], activation_angle_degrees=float(args.activation_angle_degrees))
