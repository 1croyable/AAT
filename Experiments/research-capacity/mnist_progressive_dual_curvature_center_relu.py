    ...
    def features(self, z: torch.Tensor):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i < len(self.activations):
                out, act_info = self.activations[i](out)
                info.update(act_info)
            else:
                info["act_delta"] = torch.zeros(out.shape[0], device=out.device)
                info["act_zero_frac"] = torch.zeros(out.shape[0], device=out.device)
            infos.append(info)
        return out, infos
...
def train_stage(model: CenterReLUStack, stage: int, train_loader, val_loader, device, epochs: int, lr: float, amp: bool, out_dir: Path):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch == 1 or epoch % 10 == 0 or epoch == epochs:
            print(
                f"center_relu_L{stage} epoch {epoch:04d}/{epochs} | train_acc={row['train_acc']:.4f} loss={row['train_loss']:.4f} | "
    ...
    with (out_dir / f"center_relu_L{stage}_log.csv").open("w", newline="", encoding="utf-8") as f:
...
def main():
    ...
    p.add_argument("--out-dir", type=str, default="mnist_progressive_dual_curvature_center_relu")
    ...
    print("=" * 110)
    print("Progressive dual-curvature with global-center ReLU | flattened MNIST")
    print("=" * 110)
    ...
    print("last layer always has no activation")
    ...
    for stage in range(1, args.max_depth + 1):
        ...
        print(
            f"AAT dual center_relu depth={stage} activations={len(model.activations)} "
            f"params={count_parameters(model):,} supports={sum(l.size_field.anchors.shape[0] for l in model.layers)}"
        )
        summaries.append(train_stage(model, stage, train_loader, val_loader, device, args.epochs, args.lr, args.amp, out_dir))

    with (out_dir / "summary.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["stage", "best_acc", "best_epoch", "params", "supports", "activations"])
        w.writeheader()
    ...
    for s in summaries:
        print(
            ...
            f"params={s['params']:,} | supports={s['supports']} | activations={s['activations']}"
        )
