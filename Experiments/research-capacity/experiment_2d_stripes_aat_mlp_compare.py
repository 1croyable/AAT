...
class MLP(nn.Module):
    def __init__(self, dim, hidden):
        ...
        last = dim
        for width in hidden:
            blocks.append(nn.Linear(last, width))
            blocks.append(nn.GELU())
            last = width
        blocks.append(nn.Linear(last, 2))
...
def train_aat(name, layers, dim, x_train, y_train, x_val, y_val, seed_offset):
    ...
    return {"name": name, "params": count_params(model), "best_acc": best_acc, "best_epoch": best_epoch, "best_loss": best_loss, "best_sigma": best_sigma}
...
def main():
    ...
    results = []
    results.append(train_mlp("MLP_small_2x32", [32, 32], x_train, y_train, x_val, y_val, 10))
    results.append(train_mlp("MLP_medium_3x128", [128, 128, 128], x_train, y_train, x_val, y_val, 20))
    results.append(train_aat("AAT_L4_2D_AAT_hidden_activation", 4, BASE_DIM, x_train, y_train, x_val, y_val, 30))
    results.append(train_aat("AAT_L6_2D_AAT_hidden_activation", 6, BASE_DIM, x_train, y_train, x_val, y_val, 40))
    results.append(train_aat("AAT_L4_3D_padded_swapped_activation", 4, EXPANDED_DIM, x_train_3, y_train, x_val_3, y_val, 50))
