# -*- coding: utf-8 -*-
r"""
experiment_mnist_aat_l8_sqrtk_activation.py

MNIST depth scan for AATField 2.0.

Purpose:
  - Check whether the weak MNIST result is mainly a depth problem.
  - Compare a plain linear baseline against AAT depths L1/L2/L4/L8.
...
Run:
  python experiment_mnist_aat_l8_sqrtk_activation.py
"""
...
# Depth scan. L4 is the already-tested structure; L8 tests whether deeper AAT helps.
DEPTH_LIST = [8]
EXTRA_DIMS = 1
BASE_DIM = 28 * 28
...
class IntegralTerrainLayer(nn.Module):
    ...
    def force(self, z: torch.Tensor) -> torch.Tensor:
        ...
        move = (wa - wsum * z) / (math.sqrt(float(N_INTEGRAL_POINTS)) * sigma * sigma)
        return move
...
def main():
    ...
    print("AAT MNIST L8: original c-gated activation, force divided by sqrt(K), trainable sigma")
    print(f"N_TRAIN={N_TRAIN}, N_VAL={N_VAL}, epochs={EPOCHS}, batch={BATCH_SIZE}")
    print(f"DEPTH_LIST={DEPTH_LIST} | force normalization: /sqrt(K), K={N_INTEGRAL_POINTS}")
