# -*- coding: utf-8 -*-
r"""
experiment_mnist_aat_sigma1_depth_scan.py

MNIST sigma=1 depth scan for AATField 2.0.

Purpose:
  - Remove sigma warm-start assumptions.
  - Initialize every layer with sigma=1.0 and let sigma train from epoch 0.
  - Keep the original c-gated local activation.
  - Keep the original integral normalization /K, not /sqrt(K) and not no-K.
  - Scan depth to see whether the model prefers shallower or deeper transport stacks.

Data path is fixed to:
  C:/Projets/AATField/Experiments/data/MNIST/raw

Run:
  python experiment_mnist_aat_sigma1_depth_scan.py
"""
...
# Depth scan with the clean sigma=1 initialization.
# Keep this moderate first; if L16 still improves, test L24 separately.
...
def make_depth_sigma_profile(depth: int) -> List[float]:
    """Clean diagnostic profile: every layer starts from sigma=1.0."""
    if depth <= 0:
...
def train_aat_depth(depth: int, x_train: torch.Tensor, y_train: torch.Tensor, x_val: torch.Tensor, y_val: torch.Tensor):
    ...
    model_name = f"AAT_L{depth}_MNIST_D{dim}_extra{EXTRA_DIMS}_sigma1_trainable_sigma"
    print("\n" + "=" * 80)
    print(model_name)
    print("=" * 80)
    print(
...
def main():
    ...
    print("AAT MNIST sigma=1 depth scan: original c-gated activation, original /K force, trainable sigma")
    print(f"N_TRAIN={N_TRAIN}, N_VAL={N_VAL}, epochs={EPOCHS}, batch={BATCH_SIZE}")
    print(f"DEPTH_LIST={DEPTH_LIST} | sigma_init=1.0 all layers | force normalization: /K, K={N_INTEGRAL_POINTS}")
