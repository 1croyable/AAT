...
def select_sigma_profile_from_visibility(x_train, y_train, depth):
    ...
    # If there is a single dominant band, use the high-score band around the peak.
    # This avoids pretending to know intermediate-layer data; it simply gives layers
    # different views inside the visible structural band.
    ...
    # Last fallback: spread narrowly around the best sigma in log space.
...
def main():
    ...
    print("AAT: dog-visible fixed sigma initializer, hidden local activation, last layer plain")
    ...
    print("\n" + "=" * 80)
    print("DOG VISIBLE-STRUCTURE FIXED SIGMA INITIALIZER")
    print("=" * 80)
    profiles = {}
    ...
    for depth in [1, 2, 3, 4]:
        specs.append((
            f"AAT_L{depth}_2D_dog_visible_sigma_init_fixed",
            depth,
            profiles[depth],
        ))
