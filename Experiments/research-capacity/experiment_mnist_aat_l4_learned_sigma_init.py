# -*- coding: utf-8 -*-
r"""
experiment_mnist_aat_l4_learned_sigma_init.py

AATField 2.0 MNIST first test.

Core structure:
  - Continuous integral Gaussian terrain layer
  - One shared sigma per layer, initialized from the previous learned MNIST profile and trained from epoch 0
  - Fully joint schedule: train c, sigma, and head together from the beginning
...
Run:
  python experiment_mnist_aat_l4_learned_sigma_init.py
"""
...
# Fully joint sigma training.
# Sigma is trainable from epoch 0. Use a moderate LR so it can move faster than the staged run.
SIGMA_LR = 5e-4
SIGMA_REG = 0.0
SIGMA_MIN = 0.50
SIGMA_MAX = 20.0
...
BASE_DIM = 28 * 28
DEPTH = 4
EXTRA_DIMS_LIST = [1]      # +1 was slightly better in the MNIST tests

# Warm-start from the best profile learned by the previous fully trainable run.
# Previous run ended around: [1.8732, 2.3019, 2.3397, 6.3194].
# The goal is to test whether better sigma initialization accelerates / improves training.
MANUAL_INIT_SIGMAS = [1.8732, 2.3019, 2.3397, 6.3194]
...
class IntegralTerrainLayer(nn.Module):
    def __init__(self, dim: int, sigma_init: float, anchor_source: torch.Tensor, extra_dims: int, perm: torch.Tensor):
        ...
        # Fully joint version: sigma participates in training from epoch 0.
...
def train_one(extra_dims: int, x_train: torch.Tensor, y_train: torch.Tensor, x_val: torch.Tensor, y_val: torch.Tensor):
    ...
    # Use the learned sigma profile from the previous MNIST trainable-sigma run.
    # This intentionally skips the DoG+floor initializer to test whether the learned
    # profile itself is a better starting point.
    sigmas = list(MANUAL_INIT_SIGMAS)
    if len(sigmas) != DEPTH:
        raise ValueError(f"MANUAL_INIT_SIGMAS length {len(sigmas)} must match DEPTH={DEPTH}")
    meta = {
        "source": "previous learned MNIST profile",
        "manual_sigmas": sigmas,
    }

    model_name = f"AAT_L{DEPTH}_MNIST_D{dim}_extra{extra_dims}_learned_profile_trainable_sigma_full"
    print("\n" + "=" * 80)
    print(model_name)
    print("=" * 80)
    print(
        f"dim {dim} | extra_dims {extra_dims} | layers {DEPTH} | "
        f"integral points/layer {N_INTEGRAL_POINTS} | learned-profile trainable sigmas {[round(s, 4) for s in sigmas]}"
    )
    print(
        "initializer: learned-profile warm start | "
        f"manual_sigmas {[round(v, 4) for v in meta['manual_sigmas']]}"
    )
...
def main():
    ...
    print("AAT: L4 MNIST, learned-profile sigma init, fully trainable sigma from epoch 0, hidden local activation, last plain")
