...
Only activation is changed:
  ...
  - local center r is read from the activation field response center.
  - BUT theta and tangent direction are read at the sample position z_mid, not at r.
...
Example:
  python experiment_mnist_aat_2p0_local_spin_activation_read_sample.py
  python experiment_mnist_aat_2p0_local_spin_activation_read_sample.py --K 16 --depth 3 --epochs 80
"""
...
class GaussianLocalSpinLayer(nn.Module):
    ...
    def activate(self, z_before: torch.Tensor, z_mid: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, float]]:
        ...
        # This is the pivot of the local rotated activation.
        r = self._act_field_response_center(z_before)

        # Important correction:
        # The activation field state is read at the current sample position z_mid,
        # not at the pivot r.  r says "rotate around here"; z_mid says
        # "at this sample location, what angle and tangent direction should be used?"
        theta, grad = self._act_field_potential_and_gradient(z_mid)
...
def main() -> None:
    ...
    print("Experiment: AAT 2.0 transport unchanged; activation is local spin field with theta/u read at sample point.")
    print("Model: local_spin_read_sample only. Last layer has no activation.")
    ...
    print(f"DONE local_spin_read_sample: best_acc={info['best_acc']:.4f}@{info['best_epoch']}, best_loss={info['best_loss']:.4f}, time={dt:.1f}s")
