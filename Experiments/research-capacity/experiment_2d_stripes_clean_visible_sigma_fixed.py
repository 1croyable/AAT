...
def visible_structure_scores(x, y, sigma_grid):
    """
    Compute the clean Gaussian-eye visibility score.

    Pure visibility starts from

        E_true(sigma) = sigma^d / N^2 * sum_ij <s_i, s_j>
                        exp(-||x_i-x_j||^2 / (4 sigma^2)).

    The raw score can be biased by two terms that are not real visible structure:

        1) self-point energy: i == j, where every sample sees itself;
        2) random-label baseline: finite-sample structure that would appear even
           if the same centered labels were randomly assigned to positions.

    This function uses

        E_clean = E_true_offdiag - E_random_offdiag

    where the random off-diagonal expectation is computed analytically, not by
    ...
    # Sum_i ||s_i||^2. The i == j contribution of k_ij is exactly 1.
    self_dot_sum = (s * s).sum()

    # For a random permutation of centered labels without replacement,
    # E[<s_pi(i), s_pi(j)>] for i != j equals this negative constant.
    random_offdiag_dot = -self_dot_sum / (n * (n - 1))

    scores = []
    for sigma in sigma_grid.to(DEVICE):
        k = torch.exp(-dist2 / (4.0 * sigma * sigma))

        # Raw true energy, including self-points.
        raw = (k @ s * s).sum() / (n * n)

        # Remove self-points: k_ii = 1.
        true_offdiag = raw - self_dot_sum / (n * n)

        # Analytic random-label baseline on the same kernel geometry.
        offdiag_kernel_sum = k.sum() - n
        random_offdiag = random_offdiag_dot * offdiag_kernel_sum / (n * n)

        clean = (sigma ** x.size(1)) * (true_offdiag - random_offdiag)
        scores.append(clean.detach().float().cpu())
...
class IntegralTerrainLayer(nn.Module):
    def __init__(self, dim, sigma_init):
        ...
        # Fixed sigma: initialized by the clean-visible scan, not trained.
...
def train_one(name, depth, sigma_profile, x_train, y_train, x_val, y_val):
    ...
    print(
        ...
        f"sigma_policy: fixed | init: clean-visible | init_sigmas [{init_text}] | "
...
def main():
    ...
    print("AAT: clean-visible fixed sigma initializer, hidden local activation, last layer plain")
    ...
    print("\n" + "=" * 80)
    print("CLEAN VISIBLE-STRUCTURE FIXED SIGMA INITIALIZER")
    print("=" * 80)
    profiles = {}
    ...
    for depth in [1, 2, 3, 4]:
        specs.append((
            f"AAT_L{depth}_2D_clean_visible_sigma_init_fixed",
            depth,
            profiles[depth],
        ))
