...
class IntegralTerrainLayer(nn.Module):
    def __init__(self, dim, sigma_init):
        ...
        # Fixed sigma: initialized by the visible-structure scan, not trained.
...
def train_one(name, depth, sigma_profile, x_train, y_train, x_val, y_val):
    ...
    print(
        ...
        f"sigma_policy: fixed | init: visible-structure | init_sigmas [{init_text}] | "
...
def main():
    ...
    print("AAT: visible-structure fixed sigma initializer, hidden local activation, last layer plain")
    ...
    print("\n" + "=" * 80)
    print("VISIBLE-STRUCTURE FIXED SIGMA INITIALIZER")
    print("=" * 80)
    profiles = {}
    ...
    for depth in [1, 2, 3, 4]:
        specs.append((
            f"AAT_L{depth}_2D_visible_sigma_init_fixed",
            depth,
            profiles[depth],
        ))
