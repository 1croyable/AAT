...
class DualCurvatureStack(nn.Module):
    ...
    @staticmethod
    def append_zero(z: torch.Tensor) -> torch.Tensor:
        return torch.cat([z.float(), torch.zeros(z.shape[0], 1, dtype=z.dtype, device=z.device)], dim=1)
    ...
    def features(self, z: torch.Tensor):
        ...
        for i, layer in enumerate(self.layers):
            if i > 0:
                out = self.append_zero(out)
            out, info = layer(out)
            infos.append(info)
        return out, infos

    def features_for_next_layer_init(self, z: torch.Tensor):
        out, _ = self.features(z)
        return self.append_zero(out)
...
@torch.no_grad()
def collect_next_layer_inputs(model: DualCurvatureStack, loader, device):
    model.eval()
    xs, ys = [], []
    for z, y in loader:
        z = z.to(device)
        feat = model.features_for_next_layer_init(z)
...
def main():
    ...
    p.add_argument("--out-dir", type=str, default="mnist_progressive_dual_curvature_growing_dim")
    ...
    print("=" * 110)
    print("Progressive growing-dimension dual-curvature field | flattened MNIST")
    print("=" * 110)
    print(f"device={device} seed={args.seed} train={z_train.shape[0]} val={z_val.shape[0]} base_dim={dim} depth={args.max_depth}")
    print("lift: flattened image -> [2*x-1, hidden zeros], no permutation")
    print("between layers: append one zero dimension before initializing/training the next layer")
    ...
    for stage in range(1, args.max_depth + 1):
        if stage == 1:
            ...
            init_z, init_y = collect_next_layer_inputs(model, init_loader, device)
        ...
        print(
            f"L{stage} init done in {time.time() - t0:.2f}s | dim={anchors.shape[1]} selected_per_class={meta['k']} "
            f"total_supports={anchors.shape[0]} k_info={meta['k_info']} k_geo={meta['k_geo']} sigma={float(sigma.mean().item()):.4f}"
        )
        print(f"AAT dual depth={stage} current_dim={model.current_dim} params={count_parameters(model):,} supports={sum(l.size_field.anchors.shape[0] for l in model.layers)}")
