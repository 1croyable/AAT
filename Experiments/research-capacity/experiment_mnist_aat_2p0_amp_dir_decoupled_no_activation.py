# -*- coding: utf-8 -*-
r"""
Minimal MNIST experiment for AAT 2.0-style Gaussian fields.

This script runs ONE model only:
  amp_dir_decoupled

Purpose:
  Test the idea "one field provides magnitude, another field provides direction".

Rules:
  - AAT 2.0-style k-means anchors / shared sigma / trainable anchors.
  - Two independent fields per layer:
      magnitude field: scalar potential m(z)
      direction field: gradient direction u(z)
  - Both fields are initialized from exactly the same anchors, sigma, and initial charges.
  - No activation at all.
  - Four layers by default.
  - Console output only. No scans. No saved files.

Run from:
  C:\Projets\AATField\Experiments\research-capacity

Example:
  python experiment_mnist_aat_2p0_amp_dir_decoupled_no_activation.py
  python experiment_mnist_aat_2p0_amp_dir_decoupled_no_activation.py --K 16 --depth 4 --epochs 80
"""
...
# -----------------------------
# Decoupled magnitude-direction field model
# -----------------------------
...
class AmpDirDecoupledLayer(nn.Module):
    def __init__(
        self,
        *,
        dim: int,
        ...
        if bool(spec.trainable_anchors):
            self.mag_a = nn.Parameter(a_init.clone())
            self.dir_a = nn.Parameter(a_init.clone())
        else:
            self.register_buffer("mag_a", a_init.clone())
            self.register_buffer("dir_a", a_init.clone())

        shared = float(torch.median(sigma_init.float()).item()) if sigma_init.numel() > 1 else float(sigma_init.item())
        self.mag_log_sigma = nn.Parameter(torch.tensor([math.log(max(shared, 1e-3))], dtype=torch.float32))
        self.dir_log_sigma = nn.Parameter(torch.tensor([math.log(max(shared, 1e-3))], dtype=torch.float32))

        # Same initial state, independent trainable parameters afterwards.
        c0 = init_charge.float().contiguous()
        self.mag_c = nn.Parameter(c0.clone())
        self.dir_c = nn.Parameter(c0.clone())
    ...
    def _effective_c(self, c: torch.Tensor) -> torch.Tensor:
        if self.spec.zero_mean_c:
            c = c - c.mean()
        return c

    def _potential(self, z: torch.Tensor, a: torch.Tensor, log_sigma: torch.Tensor, c: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        a = a.to(dtype=z.dtype, device=z.device)
        sigma = log_sigma.exp().to(dtype=z.dtype, device=z.device).view(1, -1)
        sigma2 = sigma.square() + EPS
        c_vec = self._effective_c(c).to(dtype=z.dtype, device=z.device).view(1, -1)
        dist2 = self._dist2(z, a)
        kernel = torch.exp(-dist2 / (2.0 * sigma2))
        phi = (kernel * c_vec).sum(dim=1, keepdim=True) / self.force_denom
        return phi, kernel

    def _gradient(self, z: torch.Tensor, a: torch.Tensor, log_sigma: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        a = a.to(dtype=z.dtype, device=z.device)
        sigma = log_sigma.exp().to(dtype=z.dtype, device=z.device).view(1, -1)
        sigma2 = sigma.square() + EPS
        c_vec = self._effective_c(c).to(dtype=z.dtype, device=z.device).view(1, -1)
    ...
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        # Magnitude from one scalar field.
        mag, _ = self._potential(z, self.mag_a, self.mag_log_sigma, self.mag_c)

        # Direction from another scalar field's gradient.
        grad = self._gradient(z, self.dir_a, self.dir_log_sigma, self.dir_c)
        grad_norm = grad.norm(dim=1, keepdim=True)
        direction = grad / grad_norm.clamp_min(1e-8)

        move = float(self.spec.eta) * mag * direction
        z_next = z + move

        debug = {
            "mag_abs": float(mag.detach().abs().mean().item()),
            "dir_grad_norm": float(grad_norm.detach().mean().item()),
            "move_abs": float(move.detach().abs().mean().item()),
            "move_norm": float(move.detach().norm(dim=1).mean().item()),
        }
        return z_next, debug


class AmpDirDecoupledModel(nn.Module):
    def __init__(
        self,
        *,
        dim: int,
        ...
        layers = []
        for li, (anchors, sigma) in enumerate(zip(anchors_by_layer, sigmas_by_layer)):
            gen = torch.Generator(device="cpu")
            gen.manual_seed(SEED + 7777 + li)
            init_charge = torch.randn(int(k_points), generator=gen, dtype=torch.float32) * C_INIT_STD
            layers.append(AmpDirDecoupledLayer(
                dim=dim,
                k_points=k_points,
                anchors=anchors,
                sigma_init=sigma,
...
def train_model(
    ...
    *,
    ...
    print("metric format per layer: mag_abs / dir_grad_norm / move_abs / move_norm")
    for epoch in range(int(epochs)):
        ...
        for x, y in train_loader:
            ...
            if metric_accum is None:
                metric_accum = [[0.0 for _ in range(4)] for _ in metrics]
...
def main() -> None:
    ...
    parser.add_argument("--extra-dims", type=int, default=0)
    ...
    print("Experiment: AAT 2.0-style decoupled magnitude-direction transport. No activation.")
    print("Model: amp_dir_decoupled only.")
    print(f"Config: K={args.K}, depth={args.depth}, extra_dims={args.extra_dims}, eta={args.eta}, epochs={args.epochs}")
    ...
    print("\n" + "=" * 88)
    print("amp_dir_decoupled")
    print("=" * 88)
    set_seed(SEED)
    spec = ModelSpec(eta=float(args.eta), zero_mean_c=True, trainable_anchors=True)
    model = AmpDirDecoupledModel(
        dim=dim,
        depth=int(args.depth),
        k_points=int(args.K),
    ...
    with torch.no_grad():
        mag_sigma = [float(layer.mag_log_sigma.exp().detach().cpu().item()) for layer in model.layers]
        dir_sigma = [float(layer.dir_log_sigma.exp().detach().cpu().item()) for layer in model.layers]
        mag_c_std = [float(layer.mag_c.detach().float().std().cpu().item()) for layer in model.layers]
        dir_c_std = [float(layer.dir_c.detach().float().std().cpu().item()) for layer in model.layers]
    print(f"mag_sigma by layer: {[round(v, 6) for v in mag_sigma]}")
    print(f"dir_sigma by layer: {[round(v, 6) for v in dir_sigma]}")
    print(f"mag_c_std by layer: {[round(v, 6) for v in mag_c_std]}")
    print(f"dir_c_std by layer: {[round(v, 6) for v in dir_c_std]}")
    print(f"DONE amp_dir_decoupled: best_acc={info['best_acc']:.4f}@{info['best_epoch']}, best_loss={info['best_loss']:.4f}, time={dt:.1f}s")
