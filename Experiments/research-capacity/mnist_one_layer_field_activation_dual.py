...
def weighted_kmeans(pts: torch.Tensor, weights: torch.Tensor, k: int, iters: int = 8):
    ...
    for _ in range(iters):
        ...
        if (new_centers - centers).norm().item() < 1e-5:
            ...
            break
        centers = new_centers
    return centers, pairwise_dist2(pts, centers).argmin(1)
...
def choose_anchors(z: torch.Tensor, y: torch.Tensor, kmeans_iters: int = 8):
    ...
    for c in range(C):
        ...
        mp = {}
        for k in needed_ks:
            centers, _ = weighted_kmeans(pts, w, k, kmeans_iters)
            mp[k] = centers.detach().clone()
...
class PotentialField(nn.Module):
    ...
    def potential(self, z: torch.Tensor):
        k, _ = self.kernel(z)
        return k @ self.charge.float()

    def grad(self, z: torch.Tensor):
        k, sigma2 = self.kernel(z)
        coef = self.charge.float()[None, :] * k / sigma2
        return coef @ self.anchors.float() - z.float() * coef.sum(1, keepdim=True)
...
class FlowUntilPositive(nn.Module):
    def __init__(self, field: PotentialField, dim: int):
        super().__init__()
        self.field = field
    ...
    def forward(self, z: torch.Tensor):
        ...
        steps = torch.zeros(zc.shape[0], device=zc.device)
        for i in range(self.max_steps):
            if i == 0:
                active = torch.ones(zc.shape[0], dtype=torch.bool, device=zc.device)
            else:
                active = self.field.potential(zc) < 0.0
            if not bool(active.any().item()):
                break
            mv = self.field.grad(zc)
            mv = mv * active.float()[:, None]
            zc = zc + mv
            total = total + mv
            steps = steps + active.float()
        phi = self.field.potential(zc).detach()
        info = {
            "move": total.detach().norm(1, dim=1),
            "steps": steps.detach(),
            "hit": (steps >= self.max_steps).float().detach(),
            "phi": phi,
            "neg": (phi < 0).float(),
        }
        return self.head(zc), info


class DualCurvatureField(nn.Module):
    def __init__(self, size_field: PotentialField, dir_field: PotentialField, dim: int):
        super().__init__()
    ...
    def forward(self, z: torch.Tensor):
        ...
        direction = self._unit(h_dir)
        move = mag * direction
        out = zf + move
        info = {"move": move.detach().norm(2, dim=1), "mag": mag.detach().squeeze(1)}
        return self.head(out), info
...
def train_one(name: str, model: nn.Module, train_loader, val_loader, device, epochs: int, lr: float, amp: bool, out_dir: Path):
    model.to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for z, y in train_loader:
            ...
            move_sum += float(info["move"].mean().item()) * bs
            if "steps" in info:
                step_sum += float(info["steps"].mean().item()) * bs
                hit_sum += float(info["hit"].mean().item()) * bs
                neg_sum += float(info["neg"].mean().item()) * bs
            elif "mag" in info:
                step_sum += float(info["mag"].mean().item()) * bs
        ...
        row = {
            ...
            "val_loss": val_loss,
            "best_acc": best,
            "move": move_sum / max(total, 1),
            "steps_or_mag": step_sum / max(total, 1),
            "hit_rate": hit_sum / max(total, 1),
            "neg_rate": neg_sum / max(total, 1),
        }
        rows.append(row)
        if epoch == 1 or epoch % 10 == 0 or epoch == epochs:
            print(
                f"{name} epoch {epoch:04d}/{epochs} | train_acc={train_acc:.4f} loss={row['train_loss']:.4f} | "
                f"val_acc={val_acc:.4f} loss={val_loss:.4f} best={best:.4f}@{best_epoch} | "
                f"move={row['move']:.4f} aux={row['steps_or_mag']:.4f} hit={row['hit_rate']:.3f} neg={row['neg_rate']:.3f}"
            )
    with (out_dir / f"{name}_log.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        w.writeheader()
        w.writerows(rows)
    return {"model": name, "best_acc": best, "best_epoch": best_epoch, "params": count_parameters(model)}
...
def main():
    ...
    p.add_argument("--out-dir", type=str, default="mnist_one_layer_two_fields")
    ...
    print("=" * 110)
    print("One-layer MNIST field experiment: flow-until-positive and dual-curvature-field")
    print("=" * 110)
    print(f"device={device} seed={args.seed} train={z_train.shape[0]} val={z_val.shape[0]} dim={dim}")
    print("lift: flattened image -> [2*x-1, hidden zeros], no permutation")

    init_z = z_train
    init_y = y_train
    if args.init_samples > 0 and args.init_samples < z_train.shape[0]:
        idx = torch.randperm(z_train.shape[0])[:args.init_samples]
        init_z = z_train[idx]
        init_y = y_train[idx]
    t0 = time.time()
    anchors, sigma, meta = choose_anchors(init_z.to(device), init_y.to(device))
    print(f"init done in {time.time() - t0:.2f}s | selected_per_class={meta['k']} total_supports={anchors.shape[0]} k_info={meta['k_info']} k_geo={meta['k_geo']} sigma={float(sigma.mean().item()):.4f}")
    ...
    flow = FlowUntilPositive(PotentialField(anchors, sigma, base_charge), dim)
    size_field = PotentialField(anchors, sigma, base_charge)
    dir_field = PotentialField(anchors, sigma, base_charge)
    dual = DualCurvatureField(size_field, dir_field, dim)

    summaries = []
    print(f"flow_until_positive params={count_parameters(flow):,}")
    summaries.append(train_one("flow_until_positive", flow, train_loader, val_loader, device, args.epochs, args.lr, args.amp, out_dir))
    print(f"dual_curvature_field params={count_parameters(dual):,}")
    summaries.append(train_one("dual_curvature_field", dual, train_loader, val_loader, device, args.epochs, args.lr, args.amp, out_dir))

    with (out_dir / "summary.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["model", "best_acc", "best_epoch", "params"])
        w.writeheader()
        w.writerows(summaries)
    print("=" * 110)
    for s in summaries:
        print(f"{s['model']} | best_acc={s['best_acc']:.4f}@{s['best_epoch']} | params={s['params']:,}")
