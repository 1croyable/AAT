...
class PotentialField(nn.Module):
    ...
    def potential(self, z: torch.Tensor):
        k, _ = self.kernel(z)
        return (k * self.charge.float()[None, :]).sum(1, keepdim=True)
...
class DualCurvatureLayer(nn.Module):
    ...
    def curvature_vector(self, field: PotentialField, z: torch.Tensor):
        ...
        return field.hvp(z, v), v

    def forward(self, z: torch.Tensor):
        zf = z.float()
        h_size, _ = self.curvature_vector(self.size_field, zf)
        h_dir, _ = self.curvature_vector(self.dir_field, zf)
    ...
    def response(self, z: torch.Tensor, lift_mode: str):
        zf = z.float()
        if lift_mode == "potential_lift":
            return self.size_field.potential(zf)
        if lift_mode == "signed_curvature_lift":
            h, v = self.curvature_vector(self.size_field, zf)
            return (h * v).sum(1, keepdim=True)
        raise ValueError(f"unknown lift_mode: {lift_mode}")


class ResponseLiftStack(nn.Module):
    def __init__(self, dim: int, lift_mode: str):
        super().__init__()
        self.base_dim = int(dim)
        self.current_dim = int(dim)
        self.lift_mode = str(lift_mode)
        self.layers = nn.ModuleList()
        self.head = nn.Linear(dim, 10)

    def expand_head(self, dim: int) -> None:
        dim = int(dim)
        if dim == self.current_dim:
            return
        old = self.head
        new = nn.Linear(dim, 10).to(old.weight.device)
        with torch.no_grad():
            keep = min(old.weight.shape[1], dim)
            new.weight[:, :keep].copy_(old.weight[:, :keep])
            new.bias.copy_(old.bias)
        self.head = new
        self.current_dim = dim

    def add_layer(self, layer: DualCurvatureLayer):
        layer_dim = int(layer.size_field.anchors.shape[1])
        if layer_dim != self.current_dim:
            raise RuntimeError(f"layer dim {layer_dim} != current dim {self.current_dim}")
        self.layers.append(layer)
        self.expand_head(layer_dim + 1)
    ...
    def features(self, z: torch.Tensor):
        ...
        for layer in self.layers:
            out, info = layer(out)
            # The response coordinate is part of the model, so keep its gradient.
            # This lets later losses shape the previous layer's response dimension.
            r = layer.response(out, self.lift_mode)
            info["response"] = r.detach().squeeze(1)
    ...
    def forward(self, z: torch.Tensor):
        ...
        response_abs = torch.zeros(z.shape[0], device=z.device)
        if infos:
            last_move = infos[-1]["move"]
            response_abs = infos[-1]["response"].abs()
            total_move = torch.stack([i["move"] for i in infos], 0).sum(0)
        return self.head(out), {"last_move": last_move, "total_move": total_move, "response_abs": response_abs}


@torch.no_grad()
def collect_features(model: ResponseLiftStack, loader, device):
    model.eval()
    xs, ys = [], []
    for z, y in loader:
        z = z.to(device)
...
def train_stage(model: ResponseLiftStack, stage: int, train_loader, val_loader, device, epochs: int, lr: float, amp: bool, out_dir: Path, tag: str):
    model.to(device)
    if stage == 1:
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        total = correct = 0
        loss_sum = move_sum = last_sum = resp_sum = 0.0
        for z, y in train_loader:
            ...
            resp_sum += float(info["response_abs"].mean().item()) * bs
        ...
        row = {
            ...
            "response_abs": resp_sum / max(total, 1),
        }
        rows.append(row)
        if epoch == 1 or epoch % 10 == 0 or epoch == epochs:
            print(
                f"{tag}_L{stage} epoch {epoch:04d}/{epochs} | train_acc={row['train_acc']:.4f} loss={row['train_loss']:.4f} | "
                f"val_acc={val_acc:.4f} loss={val_loss:.4f} best={best:.4f}@{best_epoch} | "
                f"last_move={row['last_move']:.4f} total_move={row['total_move']:.4f} resp_abs={row['response_abs']:.4f}"
            )
    with (out_dir / f"{tag}_L{stage}_log.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        w.writeheader()
        w.writerows(rows)
    return {"model": tag, "stage": stage, "best_acc": best, "best_epoch": best_epoch, "params": count_parameters(model), "supports": sum(l.size_field.anchors.shape[0] for l in model.layers), "dim": model.current_dim}


def run_model(tag: str, z_train: torch.Tensor, y_train: torch.Tensor, z_val: torch.Tensor, y_val: torch.Tensor, args, device, base_out_dir: Path):
    set_seed(args.seed)
    ...
    model = ResponseLiftStack(z_train.shape[1], tag).to(device)
    summaries = []
    print("-" * 110)
    print(f"model={tag}")
    for stage in range(1, args.max_depth + 1):
        ...
        print(
            f"{tag} L{stage} init done in {time.time() - t0:.2f}s | input_dim={anchors.shape[1]} output_dim={model.current_dim} "
            f"selected_per_class={meta['k']} total_supports={anchors.shape[0]} k_info={meta['k_info']} k_geo={meta['k_geo']} sigma={float(sigma.mean().item()):.4f}"
        )
        print(f"{tag} depth={stage} params={count_parameters(model):,} supports={sum(l.size_field.anchors.shape[0] for l in model.layers)}")
        summaries.append(train_stage(model, stage, train_loader, val_loader, device, args.epochs, args.lr, args.amp, out_dir, tag))
    with (out_dir / "summary.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["model", "stage", "best_acc", "best_epoch", "params", "supports", "dim"])
        w.writeheader()
...
def main():
    ...
    p.add_argument("--out-dir", type=str, default="mnist_progressive_dual_curvature_response_lift")
    ...
    print("=" * 110)
    print("Progressive dual-curvature response-lift comparison | flattened MNIST")
    print("=" * 110)
    print(f"device={device} seed={args.seed} train={z_train.shape[0]} val={z_val.shape[0]} base_dim={z_train.shape[1]} depth={args.max_depth}")
    print("models: signed_curvature_lift and potential_lift")
    print("between layers: append one scalar response from the previous layer field")

    all_rows = []
    for tag in ["signed_curvature_lift", "potential_lift"]:
        all_rows.extend(run_model(tag, z_train, y_train, z_val, y_val, args, device, out_dir))
    ...
    print("=" * 110)
    for s in all_rows:
        print(f"{s['model']} L{s['stage']} | best_acc={s['best_acc']:.4f}@{s['best_epoch']} | params={s['params']:,} | supports={s['supports']} | dim={s['dim']}")
