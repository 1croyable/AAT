...
class ResponseGatedDualCurvatureLayer(nn.Module):
    """
    Field-native activation layer.

    The size field produces a signed curvature response:
        s(z) = v_size^T H_size(z) v_size
    The move magnitude is ReLU(s(z)).
    The direction field produces only the transport direction:
        d(z) = unit(H_dir(z) v_dir)
    Then:
        z_out = z + ReLU(s(z)) * d(z)
    """
    ...
    def local_direction(self, field: PotentialField, z: torch.Tensor):
        center = field.center(z)
        return self.unit(z.float() - center.float())

    def curvature_vector(self, field: PotentialField, z: torch.Tensor):
        v = self.local_direction(field, z)
        h = field.hvp(z, v)
        return h, v

    def forward(self, z: torch.Tensor):
        zf = z.float()
        h_size, v_size = self.curvature_vector(self.size_field, zf)
        signed = (h_size * v_size).sum(1, keepdim=True)
        mag = F.relu(signed)
        ...
        move = mag * direction
        return zf + move, {
            "move": move.detach().norm(2, dim=1),
            "signed": signed.detach().squeeze(1),
            "mag": mag.detach().squeeze(1),
            "gate_on": (signed.detach().squeeze(1) > 0).float(),
        }


class ResponseGateStack(nn.Module):
    def __init__(self, dim: int):
        super().__init__()
        self.layers = nn.ModuleList()
        self.head = nn.Linear(dim, 10)

    def add_layer(self, layer: ResponseGatedDualCurvatureLayer):
    ...
    def forward(self, z: torch.Tensor):
        ...
        signed_abs = torch.zeros(z.shape[0], device=z.device)
        signed_mean = torch.zeros(z.shape[0], device=z.device)
        gate_on = torch.zeros(z.shape[0], device=z.device)
        if infos:
            last_move = infos[-1]["move"]
            total_move = torch.stack([i["move"] for i in infos], 0).sum(0)
            signed_abs = torch.stack([i["signed"].abs() for i in infos], 0).mean(0)
            signed_mean = torch.stack([i["signed"] for i in infos], 0).mean(0)
            gate_on = torch.stack([i["gate_on"] for i in infos], 0).mean(0)
        return self.head(out), {
            "last_move": last_move,
            "total_move": total_move,
            "signed_abs": signed_abs,
...
@torch.no_grad()
def collect_features(model: ResponseGateStack, loader, device):
    model.eval()
    xs, ys = [], []
    for z, y in loader:
        z = z.to(device)
...
def train_stage(model: ResponseGateStack, stage: int, train_loader, val_loader, device, epochs: int, lr: float, amp: bool, out_dir: Path):
    model.to(device)
    if stage == 1:
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        total = correct = 0
        loss_sum = move_sum = last_sum = signed_abs_sum = signed_mean_sum = gate_sum = 0.0
        for z, y in train_loader:
            ...
            signed_abs_sum += float(info["signed_abs"].mean().item()) * bs
            signed_mean_sum += float(info["signed_mean"].mean().item()) * bs
            gate_sum += float(info["gate_on"].mean().item()) * bs
        ...
        row = {
            ...
            "signed_abs": signed_abs_sum / max(total, 1),
            "signed_mean": signed_mean_sum / max(total, 1),
            "gate_on": gate_sum / max(total, 1),
        }
        rows.append(row)
        if epoch == 1 or epoch % 10 == 0 or epoch == epochs:
            print(
                f"response_gate_L{stage} epoch {epoch:04d}/{epochs} | train_acc={row['train_acc']:.4f} loss={row['train_loss']:.4f} | "
                f"val_acc={val_acc:.4f} loss={val_loss:.4f} best={best:.4f}@{best_epoch} | "
                f"last_move={row['last_move']:.4f} total_move={row['total_move']:.4f} "
                f"signed_abs={row['signed_abs']:.4f} signed_mean={row['signed_mean']:.4f} gate_on={row['gate_on']:.3f}"
            )
    with (out_dir / f"response_gate_L{stage}_log.csv").open("w", newline="", encoding="utf-8") as f:
...
def main():
    ...
    p.add_argument("--out-dir", type=str, default="mnist_progressive_dual_curvature_response_gate")
    ...
    print("=" * 110)
    print("Progressive response-gated dual-curvature field | flattened MNIST")
    print("=" * 110)
    ...
    print("layer rule: magnitude = ReLU(v_size^T H_size v_size), direction = unit(H_dir v_dir)")
    print("no external activation, no append dimension, one model only")

    model = ResponseGateStack(dim).to(device)
    summaries = []
    ...
    for stage in range(1, args.max_depth + 1):
        ...
        model.add_layer(ResponseGatedDualCurvatureLayer(anchors, sigma, base_charge).to(device))
        print(
        ...
        print(
            f"AAT response-gated dual depth={stage} params={count_parameters(model):,} "
    ...
    for s in summaries:
        print(
            ...
            f"params={s['params']:,} | supports={s['supports']}"
        )
