# -*- coding: utf-8 -*-
"""
experiment_mnist_aat_l4_dog_floor.py

AATField 2.0 MNIST first test.

Core structure:
  - Continuous integral Gaussian terrain layer
  - One shared fixed sigma per layer
  - Sigma initialized by DoG / center-surround visible-structure scan with resolution floor
...
Run:
  python experiment_mnist_aat_l4_dog_floor.py
"""
...
BASE_DIM = 28 * 28
DEPTH = 4
EXTRA_DIMS_LIST = [1, 2]   # test +1 and +2 extra dimensions
...
def train_one(extra_dims: int, x_train: torch.Tensor, y_train: torch.Tensor, x_val: torch.Tensor, y_val: torch.Tensor):
    ...
    model_name = f"AAT_L{DEPTH}_MNIST_D{dim}_extra{extra_dims}_dog_floor_fixed_sigma"
    print("\n" + "=" * 80)
    print(model_name)
    print("=" * 80)
    print(
        f"dim {dim} | extra_dims {extra_dims} | layers {DEPTH} | "
        f"integral points/layer {N_INTEGRAL_POINTS} | fixed sigmas {[round(s, 4) for s in sigmas]}"
    )
    print(
        "initializer: "
...
def main():
    ...
    print("AAT: L4 MNIST, DoG+floor fixed sigma, hidden local activation, last plain")
