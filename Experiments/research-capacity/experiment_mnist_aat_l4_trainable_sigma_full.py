# -*- coding: utf-8 -*-
r"""
experiment_mnist_aat_l4_trainable_sigma_full.py

AATField 2.0 MNIST first test.

Core structure:
  - Continuous integral Gaussian terrain layer
  - One shared sigma per layer, initialized by DoG+floor and trained from epoch 0
...
Run:
  python experiment_mnist_aat_l4_trainable_sigma_full.py
"""
...
def train_one(extra_dims: int, x_train: torch.Tensor, y_train: torch.Tensor, x_val: torch.Tensor, y_val: torch.Tensor):
    ...
    model_name = f"AAT_L{DEPTH}_MNIST_D{dim}_extra{extra_dims}_dog_floor_trainable_sigma_full"
    print("\n" + "=" * 80)
    print(model_name)
    print("=" * 80)
    print(
        f"dim {dim} | extra_dims {extra_dims} | layers {DEPTH} | "
        f"integral points/layer {N_INTEGRAL_POINTS} | fully trainable sigmas {[round(s, 4) for s in sigmas]}"
    )
    print(
        "initializer: "
...
def main():
    ...
    print("AAT: L4 MNIST, DoG+floor init, fully trainable sigma from epoch 0, hidden local activation, last plain")
