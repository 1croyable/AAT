...
def make_checkerboard(n: int, grid_size: int = 4, seed: int = 0):
    ...
    x = torch.rand((int(n), 2), generator=gen)
...
def main() -> None:
    ...
    x_train, y_train = make_checkerboard(n_train, grid_size=grid_size, seed=seed)
    x_val, y_val = make_checkerboard(n_val, grid_size=grid_size, seed=seed + 1)
    x_train, y_train = x_train.to(device), y_train.to(device)
    x_val, y_val = x_val.to(device), y_val.to(device)

    cfg = AATConfig(input_dim=2, num_classes=2, layers=8, rays=32, kappa=6.0, ray_dropout=0.15)
