...
def make_swissroll(
    n: int,
    classes: int = 3,
    stripe_repeats: int = 2,
    t_min: float = 1.5 * math.pi,
    t_max: float = 4.8 * math.pi,
    height: float = 2.0,
    noise: float = 0.04,
    seed: int = 0,
):
    ...
    t = torch.rand((int(n),), generator=gen) * (float(t_max) - float(t_min)) + float(t_min)
    h = (torch.rand((int(n),), generator=gen) - 0.5) * float(height)
    ...
    pts[:, 0] = pts[:, 0] / scale
    pts[:, 2] = pts[:, 2] / scale
    pts[:, 1] = pts[:, 1] / max(float(height) * 0.5, 1e-8)

    pts = pts + torch.randn(pts.shape, generator=gen) * float(noise)

    total_stripes = int(classes) * int(stripe_repeats)
    phase = (t - float(t_min)) / max(float(t_max) - float(t_min), 1e-8)
    stripe = torch.floor(phase * float(total_stripes)).long().clamp(max=total_stripes - 1)
    y = stripe.remainder(int(classes))

    perm = torch.randperm(pts.shape[0], generator=gen)
    return pts.index_select(0, perm), y.index_select(0, perm)
...
def main() -> None:
    ...
    out_path = out_dir / "swissroll_3d.pt"
    ...
    x_train, y_train = make_swissroll(n_train, seed=seed)
    x_val, y_val = make_swissroll(n_val, seed=seed + 1)
    x_train, y_train = x_train.to(device), y_train.to(device)
    x_val, y_val = x_val.to(device), y_val.to(device)

    cfg = AATConfig(input_dim=3, num_classes=3, layers=8, rays=32, kappa=6.0, ray_dropout=0.15)
